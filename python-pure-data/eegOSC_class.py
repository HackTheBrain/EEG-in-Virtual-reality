...
class eeg_to_osc(object):
    ...
    def __init__(self, source = "online", serial_number = None, input_file = None, sampling_rate = 128, channel_list = None, buffer_size = None, dump_online_file = False):
        ...
        if self.source == 'online':
            ...
            self.sampling_rate = sampling_rate
            self.osc_data_queue = dict(zip(self.channel_list, [mp.Queue() for i in range(len(self.channel_list))]))
    ...
    def _osc_server_handler(self, addr, tags, data, source):
        ...
        ch = addr[1:]
        self.osc_data_queue[ch].put(data)
        # print(ch, 'put')
    ...
    def _thread_process_osc(self):
        ...
        time.sleep(1)
        ticker_start = time.time()
        buffer_data = dict(zip(self.channel_list, [[] for i in range(len(self.channel_list))]))
        try:
            while True:
                if ((time.time() - ticker_start) % 1) == 0:
                    for ch in self.channel_list:
                        for i in range(self.sampling_rate):
                            buffer_data[ch].append(self.osc_data_queue[ch].get())
                        self._osc_msg(address = '/%s/spec_c' % (ch), 
                            msg = self.spectral_centroid(ts = np.array(buffer_data[ch])[:, 0], N = len(buffer_data[ch])))
                        self._osc_msg(address = '/%s/hj_act' % (ch), 
                            msg = self.hjorth_activity(ts = np.array(buffer_data[ch])[:, 0], N = len(buffer_data[ch])))
                        self._osc_msg(address = '/%s/hj_mob' % (ch), 
                            msg = self.hjorth_mobility(ts = np.array(buffer_data[ch])[:, 0], N = len(buffer_data[ch])))
                        self._osc_msg(address = '/%s/hj_com' % (ch), 
                            msg = self.hjorth_complexity(ts = np.array(buffer_data[ch])[:, 0], N = len(buffer_data[ch])))
